...
#define AOS_WRITE_FAIL      -4
#define AOS_WRITE_FAIL_2    -5
#define AOS_CREATE_FAIL     -6
#define AOS_READ_FAIL       -7
#define AOS_SEEK_FAIL_2     -8
#define AOS_STAT_FAIL       -9
#define AOS_CLOSE_FAIL      -10
...
void updateContainers(string, int, int);        //done
void errorOut(int);
        ...
        if(disp != opened.size)
        {
            ...
            else //if(freeOfBlock != 0)
            {
                ...
                for(int i = 0; i < 10; i++)
                {
                    ...
                    if(blockNum == -1)
                    {
                        cout << "Failed to Allocate Space in Fat. FAT IS FULL!" << endl;
                        return AOS_SUCCESS;
                    }
                    if(buff_size < 100)
                    {
...
void errorOut(int rc)
{

    if( rc == AOS_PRINT_FAIL )
        cout << "Failed to print buffer: Buffer does not exist." << endl;
    if( rc == AOS_OPEN_FAIL)
        cout << "Failed to open file: Make sure File is created first." << endl;
    if( rc == AOS_SEEK_FAIL)
        cout << "Failed to seek: Seeked past end of file." << endl;
    if( rc == AOS_WRITE_FAIL)
        cout << "Failed to Write: There is no open file;!" << endl;
    if( rc == AOS_CREATE_FAIL)
        cout << "Failed to Create: Fat is full!" << endl;
    if(rc == AOS_READ_FAIL)
        cout << "Failed to Read: There is no open file!" << endl;
    if(rc == AOS_SEEK_FAIL_2)
        cout << "Failed to Seek: There is no open file!" << endl;
    if(rc == AOS_STAT_FAIL)
        cout << "Failed to Stat: There is no open file!" << endl;
    if(rc == AOS_CLOSE_FAIL)
        cout << "Failed to Close: There is no open file!" << endl;
